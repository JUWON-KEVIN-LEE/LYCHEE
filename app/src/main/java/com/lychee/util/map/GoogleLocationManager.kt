    ...
    private val mLocationRequest: LocationRequest = LocationRequest().apply {
        ...
        priority = when {
            (mContext.getSystemService(Context.LOCATION_SERVICE) as LocationManager)
                    .isProviderEnabled(LocationManager.GPS_PROVIDER) -> LocationRequest.PRIORITY_HIGH_ACCURACY

            (mContext.getSystemService(Context.LOCATION_SERVICE) as LocationManager)
                    .isProviderEnabled(LocationManager.NETWORK_PROVIDER) -> LocationRequest.PRIORITY_BALANCED_POWER_ACCURACY
                            ...
                            ?.let {statusCode ->
                                when(statusCode) {
                                    CommonStatusCodes.RESOLUTION_REQUIRED -> {
                                        val resolvable = exception as? ResolvableApiException
                                        resolvable
                                                ?.startResolutionForResult(context as Activity, REQUEST_CHECK_SETTINGS)
                                    }
